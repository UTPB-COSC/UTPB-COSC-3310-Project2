...
_start:
    mov r10, 1           ; initialize loop counter to 1

loop_head:
    cmp r10, 100              ; checks to ensure r10 is less than 100
...
; checks if divisible by 3
    ...
    mov rcx, 3               ; divisor (3)
    div rcx                  ; performs division: rax = rax / 3, rdx = remainder
    cmp rdx, 0               ; checks if remainder is zero
    je is_not_prime_10          ; if divisible by 3, it's not prime

    ; checks if divisible by 5
    mov rax, r10 ; makes sure rax is r10
    mov rdx, 0               ; clear rdx (holds remainder)
    mov rcx, 5               ; divisor (5)
    div rcx                  ; performs division: rax = rax / 5, rdx = remainder
    cmp rdx, 0               ; checks if remainder is zero
